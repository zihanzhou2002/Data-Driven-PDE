...
def TrainSTLasso(R, Ut, lam, d_tol, maxit = 25, STR_iters = 10, l0_penalty = None, normalize = 2, split = 0.8, print_best_tol = False):
    """
    This function trains a predictor using Lasso.
    ...
    for iter in range(maxit):
        ...
        # Get a set of coefficients and error
        w = STLasso(TrainR,TrainY,lam,STR_iters,tol,normalize = normalize)
        err = np.linalg.norm(TestY - TestR.dot(w), 2) + l0_penalty*np.count_nonzero(w)
...
def TrainSTElasticNet(R, Ut, lam1,lam2, d_tol, maxit = 25, STR_iters = 10, l0_penalty = None, normalize = 2, split = 0.8, print_best_tol = False):
    """
    This function trains a predictor using Lasso.
    ...
    for iter in range(maxit):
        ...
        # Get a set of coefficients and error
        w = STElasticNet(TrainR,TrainY,lam1,lam2, STR_iters,tol,normalize = normalize)
        err = np.linalg.norm(TestY - TestR.dot(w), 2) + l0_penalty*np.count_nonzero(w)
...
def Lasso(X0, Y, lam, w = np.array([0]), maxit = 100, normalize = 2):
    ...
    for iters in range(0, maxit):
        ...
        for j in range(d): 
            ...
            if len(z[j]) == 1:
                #print("1d")
                w[j] = np.multiply(np.sign(z[j])[0], np.max([abs(z[j])[0]-lam/L,0]))
            else:
                #print("multi-D")
    ...
    for iters in range(0, maxit):
        ...
        for j in range(d): 
            if len(z[j]) == 1:
                ...
                #print("multi-D")
                w[j] = np.multiply(np.sign(z[j]), np.max([abs(z[j])-max(lam1, lam2)/L,0]))
...
def STLasso(X0, y, lam, maxit, tol, normalize = 2, print_results = False):
    """
    Sequential Threshold Lasso Regression algorithm for finding (hopefully) sparse 
    ...
    for j in range(maxit):
        ...
        w[smallinds] = 0
        if lam != 0: w[biginds] = Lasso(X[:, biginds], y,lam)
        else: w[biginds] = np.linalg.lstsq(X[:, biginds],y,rcond=None)[0]

    # Now that we have the sparsity pattern, use standard least squares to get w
    if len(biginds) > 0: w[biginds] = Lasso(X[:, biginds],y,lam)
...
def STElasticNet(X0, y, lam1,lam2, maxit, tol, normalize = 2, print_results = False):
    """
    Sequential Threshold Lasso Regression algorithm for finding (hopefully) sparse 
    approximation to X^{-1}y.  The idea is that this may do better with correlated observables.
    ...
    # Get the standard 
    if lam1 != 0 or lam2 !=0: w = ElasticNet(X, y, lam1,lam2)
    else: w = np.linalg.lstsq(X,y,rcond=None)[0]
    num_relevant = d
    biginds = np.where(abs(w) > tol)[0]
    ...
    for j in range(maxit):
        ...
        # Otherwise get a new guess
        w[smallinds] = 0
        if lam1 != 0 or lam2 != 0: w[biginds] = ElasticNet(X[:, biginds], y,lam1, lam2)
        else: w[biginds] = np.linalg.lstsq(X[:, biginds],y,rcond=None)[0]

    # Now that we have the sparsity pattern, use standard least squares to get w
    if len(biginds) > 0: w[biginds] = ElasticNet(X[:, biginds],y,lam1, lam2)
